...
const NavBar = () => {
  ...
  const menuItemVariants = { hidden: { x: -20, opacity: 0 }, visible: (i) => ({ x: 0, opacity: 1, transition: { delay: i * 0.1, duration: 0.3 } }) };

  return (
    <>
      <div className={`w-full mx-auto py-4 px-4 xl:px-0 relative z-50 ${!navStyle && "bg-blue-800"}`}>
        <div className="max-w-[1280px] flex justify-between items-center mx-auto">
          <RiMenu2Fill className="text-white text-2xl cursor-pointer md:hidden" onClick={() => setMobileMenuOpen(true)} />
          <div className="hidden md:flex items-center gap-7">
            <img src="/white-logo.png" alt="logo" className="w-44 h-auto" />
            <div className="flex items-center gap-7">
              <Link to="/" className="text-white hover:text-gray-300 text-md">Начало</Link>
              <Link to="/ads" className="text-white hover:text-gray-300 text-md">Услуги</Link>
              <Link to="/ads" className="text-white hover:text-gray-300 text-md">Блог</Link>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div ref={profileRef} onClick={() => setAuthModal((prev) => !prev)} className="items-end cursor-pointer flex">
          ...
          <div className="absolute top-16 right-4 xl:right-[120px]" ref={dropdownRef}>
            {accessToken ? <ProfileDropDown isOpen={authModal} /> : <AuthDropDown isOpen={authModal} />}
          </div>
        </div>
      </div>
            ...
            >
              <div className="flex flex-col h-full">
                <div className="flex justify-between items-center p-6 border-b border-blue-700">
                  <img src="/white-logo.png" alt="logo" className="w-36 h-auto" />
                  <RiCloseLine className="text-white text-3xl cursor-pointer hover:text-gray-300" onClick={() => setMobileMenuOpen(false)} />
                </div>
                <div className="flex flex-col p-6 space-y-6">
                  {[
                    ...
                    <motion.div key={item.to} variants={menuItemVariants} initial="hidden" animate="visible" custom={index}>
                      <Link to={item.to} className="text-white hover:text-gray-300 text-lg font-medium block py-2 transition-colors duration-200" onClick={() => setMobileMenuOpen(false)}>
                        {item.label}
                      </Link>
                    </motion.div>
                  ))}
